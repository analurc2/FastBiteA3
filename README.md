# FastBiteA3
FastBite é um sistema de fast-food desenvolvido para o projeto A3 da máteria de Algoritmos e Programação, com foco na gestão de pedidos, clientes e produtos. A aplicação busca otimizar o atendimento e organizar o fluxo de vendas, proporcionando uma experiência prática e eficiente tanto para os usuários quanto para os administradores do sistema.

## 🛠️ Ferramentas

**Linguagem**
- Java

**Ferramentas de desenvolvimento**
- Git & GitHub
- VSCode

---

## 📁 Estrutura do Projeto

```
FastBite/
├── A_MenuInicial/
│   ├── FastBite.java       # Ponto de entrada e menu inicial
│   ├── Menuapp.java        # Menu principal pós-login
│   ├── Compra.java         # Fluxo de compra
│   └── Cores.java          # Cores do terminal
│
├── B_Pedido/
│   ├── Carrinho.java       # Gerenciamento do carrinho
│   ├── Finalizacao.java    # Finalização e pagamento
│   ├── Historico.java      # Histórico de pedidos
│   ├── Perfil.java         # Perfil do usuário
│   └── Restaurantes.java   # Listagem de restaurantes
│
└── C_Cardapios/
    ├── CardapioMcDonalds.java
    ├── CardapioPizza.java
    ├── CardapioAcai.java
    └── CardapioDogao.java
```

---

## 👥 Integrantes

| Nome | RA |
|---|---|
| Ana Luiza Rodrigues Carvalho | 82619364 |
| Pedro Henrique Lazarine de Freitas | 826196840 |
| Gabriel da Silva Nóbrega | 826175342 |
| Vinicius Marcantonio Vieira | 826129742 |
| Gabriel Ferreira da Silva Lima | 826199697 |


## 🎓 Informações Acadêmicas

**Disciplina:** Algoritmos e Programação  
**Instituição:** Universidade São Judas Tadeu  
**Professora:** Andréia Máchion
